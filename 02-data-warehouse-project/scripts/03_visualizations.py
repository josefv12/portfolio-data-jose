...
La conexión a PostgreSQL se obtiene desde la variable de entorno
DATABASE_URL. No se almacenan credenciales en el repositorio.
"""
...
def get_engine():
    """Create the SQLAlchemy engine using the environment configuration."""
    return create_engine(DATABASE_URL)


def save(fig, name):
    """Save a matplotlib figure in the project's screenshots directory."""
...
def chart_monthly_revenue(engine):
    ...
    ax1.set_title("Revenue mensual y ticket promedio — Online Retail II", fontsize=13, pad=14)
    fig.tight_layout()
...
def chart_pareto(engine):
    df = pd.read_sql(
        ...
        "FROM retail.vw_top_products ORDER BY revenue_rank",
        engine,
    )
    ...
    ax1.bar(
        df_plot["revenue_rank"],
        df_plot["revenue"] / 1000,
        color=PALETTE[0],
        alpha=0.7,
        width=1,
    )
    ...
    ax2 = ax1.twinx()
    ax2.plot(df_plot["revenue_rank"], df_plot["cumulative_pct"], color=PALETTE[2], linewidth=2)
    ...
    ax1.set_title("Análisis Pareto — concentración de revenue en productos", fontsize=13, pad=14)
    fig.tight_layout()
...
def chart_rfm_segments(engine):
    ...
    ax2.bar(range(len(df)), df["avg_revenue"], color=colors, alpha=0.85)
